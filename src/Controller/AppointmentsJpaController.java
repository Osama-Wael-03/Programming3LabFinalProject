...
public class AppointmentsJpaController implements Serializable {
    ...
    public List<Appointments> findBookedAppointmentsEntities() {
    EntityManager em = getEntityManager();
    try {
        CriteriaBuilder cb = em.getCriteriaBuilder();
        CriteriaQuery<Appointments> cq = cb.createQuery(Appointments.class);
        Root<Appointments> root = cq.from(Appointments.class);
        cq.select(root).where(cb.equal(root.get("status"), "booked"));
        Query q = em.createQuery(cq);
        return q.getResultList();
    } finally {
        em.close();
    ...
    public List<Object[]> fetchAppointmentsWithFirstName() {
    EntityManager em = null;
    try {
        em = getEntityManager();
        Query query = em.createQuery("SELECT a.id, a.appointmentDate, a.appointmentDay, a.appointmentTime, u.firstname " +
                "FROM Appointments a JOIN a.bookedAppointmentsCollection b JOIN b.userId u");
